...
class SQLiteDatabase {
    ...
    init(){
        ...
        do {
            ...
            //GAMES
            do{
                try db.run(games.insertMany([[id <- 2307, title <- "Crown Room Kings vs Townline Tunnelers", home <- 1371, away <- 1370, homeScore <- 2, awayScore <- 2, date <- "2020-10-07", time <- "22:00:05", location <- 42],[id <- 2313, title <- "Atlas Steelers vs Merritt Islanders", home <- 940, away <- 1824, homeScore <- 1, awayScore <- 9, date <- "2020-10-07", time <- "22:00:48", location <- 35], [id <- 2316, title <- "Dain City Dusters vs Lincoln Street Legends", home <- 1810, away <- 1822, homeScore <- 3, awayScore <- 1, date <- "2020-10-07", time <- "21:15:19", location <- 30]]))
            }
            catch {
                print(error)
            }
